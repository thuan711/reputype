...
foreach ($titleElements as $key => $element) {
    ...
    if ($parentElement->previousSibling->getAttribute('class') !== 'article-item') {
        continue; // Nếu không khớp, bỏ qua và tiếp tục vòng lặp
    }
   
    $title = $element->textContent;
    // $description = $element-> nextSibling->textContent;
    $image = $element->parentNode->previousSibling-> childNodes[0]->childNodes[1]->childNodes[2]->getAttribute('data-src');
    $data[] = [
        'title' => $title,
